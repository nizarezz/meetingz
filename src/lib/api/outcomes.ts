...
export const outcomesApi = {
  ...
  create: (meetingId: string, input: { primary_outcome: PrimaryOutcome; action_items?: ActionItem[] }) =>
    api().post(`outcomes/${meetingId}`, { json: input }).json<Outcome>(),
  update: (meetingId: string, input: Partial<{ primary_outcome: PrimaryOutcome; action_items: ActionItem[] }>) =>
